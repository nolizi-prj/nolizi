...
function repoRoot(from: string): string {
  let dir = from;
  for (let i = 0; i < 8; i++) {
    if (existsSync(resolve(dir, 'spec/0001-scheduling-core/acceptance/cases.json'))) return dir;
    dir = dirname(dir);
  }
  throw new Error('could not locate the repository root from ' + from);
}
const here = dirname(fileURLToPath(import.meta.url));
const SUITE = resolve(repoRoot(here), 'spec/0001-scheduling-core/acceptance/cases.json');
